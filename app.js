...
async function loadAllPlayersForSearch() {
  ...
  if (players) window.allPlayers = players;
}
...
window.showHistory = () => {
  ...
  if (historyList) {
    historyList.innerHTML = '<div style="color:var(--gold); text-align:center; padding:20px; font-weight:bold;">Загрузка истории матчей...</div>';
  }
  
  // Асинхронный вызов через 50 миллисекунд разрывает сетевой тупик
  setTimeout(() => {
    if (typeof loadHistory === 'function') {
      loadHistory();
    } else {
      if (historyList) {
        historyList.innerHTML = '<div style="color:#444; text-align:center; padding:20px;">История пуста</div>';
      }
    }
  }, 50);
};
